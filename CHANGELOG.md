...
### Works With CodeTour Files

- **CodeTour import** — Side Bae now detects Microsoft CodeTour files (`*.tour` / `aka.ms/codetour-schema`) and auto-converts them to its native graph format. A tour generated by any AI tool (Claude, Cursor, Copilot, ChatGPT) loads correctly even when it comes out in CodeTour's shape.
- **Hardened tour skills** — `/side-bae-tour` and `/side-bae-investigate` now emit Side Bae's format explicitly (never CodeTour), verify line numbers instead of guessing, and stamp real timestamps. Feature-vs-concept discovery is disambiguated, and "What's New" dates are stored absolute and rendered relative so they never go stale.
